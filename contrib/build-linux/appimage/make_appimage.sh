...
cp -f "$PROJECT_ROOT/electrum_mona/libsecp256k1.so.0" "$APPDIR/usr/lib/libsecp256k1.so.0" || fail "Could not copy libsecp to its destination"
...
(
    ...
    rm -rf "$PROJECT_ROOT/electrum_mona/locale/"
    for i in ./locale/*; do
