...
from electrum_mona.util import bh2u, create_and_start_event_loop, NetworkRetryManager, bfh, OldTaskGroup
...
class TestPeer(TestCaseForTestnet):
    ...
    def test_reestablish_with_old_state(self):
        ...
        async def f():
            await gath
        with self.assertRaises(electrum_mona.lnutil.RemoteMisbehaving):
            run(f())
