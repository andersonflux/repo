...
from electrum_mona import storage, bitcoin, keystore, bip32, slip39, wallet
...
#        self.assertEqual(w.get_change_addresses()[0], 'bc1qd4q50nft7kxm9yglfnpup9ed2ukj3tkxp793y0zya8dc9m39jcwq308dxz')
    ...
    def test_bip39_seed_bip44_standard(self, mock_save_db):
        ...
        ks = keystore.from_bip43_rootseed(root_seed, "m/44'/22'/0'")
    ...
    def test_bip39_seed_bip49_p2sh_segwit(self, mock_save_db):
        ...
        root_seed = keystore.bip39_to_seed(seed_words, '')
        ks = keystore.from_bip43_rootseed(root_seed, "m/49'/22'/0'")
    ...
    def test_bip39_seed_bip84_native_segwit(self, mock_save_db):
        ...
        root_seed = keystore.bip39_to_seed(seed_words, '')
        ks = keystore.from_bip43_rootseed(root_seed, "m/84'/22'/0'")
    ...
    def test_slip39_basic_3of6_bip44_standard(self, mock_save_db):
        ...
        self.assertEqual(w.get_receiving_addresses()[0], 'MVhvYbNnHcLFqGNEVGRggmP8KXKFUeDjvn')
        self.assertEqual(w.get_change_addresses()[0], 'MHqEBFSGfzQ6LMKu4NHuuDQWMXiSkDz21i')
    ...
    def test_slip39_basic_2of5_bip49_p2sh_segwit(self, mock_save_db):
        ...
        self.assertEqual(w.get_receiving_addresses()[0], 'PP6qcEQuzrfSarvrBqSSeZJGp7YpqXqjMw')
        self.assertEqual(w.get_change_addresses()[0], 'PNQ5rrbpcskRTNjx9pv75nkyn2GHzTJFP5')
    ...
    def test_slip39_groups_128bit_bip84_native_segwit(self, mock_save_db):
        ...
        self.assertEqual(w.get_receiving_addresses()[0], 'mona1qaggygkqgqjjpt58zrmhvjz5m9dj8mjsh2ufr9a')
        self.assertEqual(w.get_change_addresses()[0], 'mona1q8l6hcvlczu4mtjcnlwhczw7vdxnvwccpkv86rr')
    ...
    def test_slip39_groups_256bit_bip49_p2sh_segwit(self, mock_save_db):
        ...
        self.assertEqual(w.get_receiving_addresses()[0], 'PNhzz3kdnhv8HigrfsEj9FHY87JS1PUq1B')
        self.assertEqual(w.get_change_addresses()[0], 'P9o5hC5kAMyUnooCHRAkiZoQ8eyJfFdcSw')
...
class TestWalletSending(TestCaseForTestnet):
    ...
    def _bump_fee_p2wpkh_decrease_payment(self, *, simulate_moving_txs, config):
        ...
        if simulate_moving_txs:
            partial_tx = tx.serialize_as_bytes().hex()
            self.assertEqual("",
                             partial_tx)
            tx = tx_from_any(partial_tx)  # simulates moving partial txn between cosigners
        self.assertFalse(tx.is_complete())
        ...
        self.assertTrue(tx.is_segwit())
        tx_copy = tx_from_any(tx.serialize())
        self.assertEqual('',
                         str(tx_copy))
        self.assertEqual('054b1576f46a834bc53e95b4e9dc6e578dd4a197db1d04dbd20341d4033ab2cc', tx_copy.txid())
        self.assertEqual('b22bc6c1f0d17681dbfb1f1eac1086b9458fa8af4448ff3300239a391290d7ec', tx_copy.wtxid())
    ...
    def _bump_fee_p2wpkh_decrease_payment_batch(self, *, simulate_moving_txs, config):
        ...
        if simulate_moving_txs:
            partial_tx = tx.serialize_as_bytes().hex()
            self.assertEqual("",
                             partial_tx)
            tx = tx_from_any(partial_tx)  # simulates moving partial txn between cosigners
        self.assertFalse(tx.is_complete())
        ...
        self.assertTrue(tx.is_segwit())
        tx_copy = tx_from_any(tx.serialize())
        self.assertEqual('',
                         str(tx_copy))
        self.assertEqual('5f11e54e4d962f96980b2dde8dc1e82d540bf1def6dbcde580eeedce1d3f715f', tx_copy.txid())
        self.assertEqual('1dc5b7a74af80702f40db352015e3c9967a6c8af40a5baa4545be2459efba084', tx_copy.wtxid())
    ...
    def test_wallet_history_chain_of_unsigned_transactions(self, mock_save_db):
        ...
        # create tx1
        outputs = [PartialTxOutput.from_address_and_value('tmona1qsfcddwf7yytl62e3catwv8hpl2hs9e367y8w6h', 100000)]
        coins = wallet.get_spendable_coins(domain=None)
        ...
        outputs = [PartialTxOutput.from_address_and_value('tmona1qq0lm9esmq6pfjc3jls7v6twy93lnqcs8zqq9te', '!')]
        coins = wallet.get_spendable_coins(domain=None)
        tx = wallet.make_unsigned_transaction(coins=coins, outputs=outputs, fee=5000)
        tx.set_rbf(True)
        tx.locktime = 1938863
    ...
    def test_wallet_does_not_create_zero_input_tx(self, mock_save_db):
        ...
        with self.subTest(msg="no coins to use as inputs, max output value, zero fee"):
            outputs = [PartialTxOutput.from_address_and_value('tmona1qsfcddwf7yytl62e3catwv8hpl2hs9e367y8w6h', '!')]
            coins = wallet.get_spendable_coins(domain=None)
        ...
        with self.subTest(msg="funded wallet, zero output value, zero fee"):
            outputs = [PartialTxOutput.from_address_and_value('tmona1qsfcddwf7yytl62e3catwv8hpl2hs9e367y8w6h', 0)]
            coins = wallet.get_spendable_coins(domain=None)
            tx = wallet.make_unsigned_transaction(coins=coins, outputs=outputs, fee=0)
            self.assertEqual(1, len(tx.inputs()))
